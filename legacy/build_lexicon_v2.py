...
class HistoricalQueryEngine:
    ...
    - applique ensuite des filtres souples sur les métadonnées :
        ...
        * --year   → entities_years
    - si aucun résultat ne passe les filtres mais qu'il y a des filtres,
      on tombe en repli sur les meilleurs résultats sémantiques.
    """
    ...
    def ask(
        ...
        3) Si aucun résultat filtré mais des filtres sont fournis :
           - on renvoie les meilleurs résultats sémantiques non filtrés
             (avec un avertissement imprimé dans la console)
        """
        ...
        # 4) Aucun résultat filtré, mais des filtres étaient demandés :
        #    on tombe en repli (fallback) sur les meilleurs résultats sémantiques.
        if person or place or year:
            print("⚠️ Aucun résultat ne satisfait strictement les filtres d'entités ;")
            print("   affichage des meilleurs résultats sémantiques sans filtrage.\n")
            ...
            base_results.sort(key=lambda r: r["distance"])
            return base_results[:top_k]

        # 5) Pas de filtres ET aucun résultat renvoyé par Chroma
        return []
